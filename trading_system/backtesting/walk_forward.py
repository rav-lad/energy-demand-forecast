...
class WalkForwardAnalyzer:
    ...
    def _generate_periods(
        ...
        while True:
            ...
            if (
                len(train_data) >= self.config.min_train_samples
                and len(test_data) >= self.config.min_test_samples
            ):
    ...
    def _optimize_parameters(
        ...
        logger.info(
            f"Optimization complete. Best {self.config.optimization_metric}: {best_metric:.4f}"
        )
    ...
    def run(
        self, data: pd.DataFrame, strategy_func: Callable, param_grid: Dict[str, List[Any]]
    ) -> Dict[str, Any]:
        """
        ...
        if len(period_tuples) == 0:
            raise ValueError(
                "No valid walk-forward periods generated. Check data length and config."
            )
        ...
        for i, (train_start, train_end, test_start, test_end) in enumerate(period_tuples):
            ...
            logger.info(f"{'='*80}")
            logger.info(
                f"Train: {train_start.date()} to {train_end.date()} ({(train_end-train_start).days} days)"
            )
            logger.info(
                f"Test:  {test_start.date()} to {test_end.date()} ({(test_end-test_start).days} days)"
            )
            ...
            train_metrics = next((t["metrics"] for t in trials if t["params"] == best_params), {})
            ...
            logger.info(
                f"  Efficiency Ratio: {test_sharpe/train_sharpe:.3f}"
                if train_sharpe != 0
                else "  Efficiency Ratio: N/A"
            )
        ...
        data_text = [
            ...
            (
                ...
                if results["efficiency_ratio"] > 0.7
                    ...
                    f"ER < 0.5: Overfitting ⚠"
                    if results["efficiency_ratio"] < 0.5
                    else f"ER 0.5-0.7: Acceptable"
                )
            ),
        ]
