...
def smape(y_true: np.ndarray, y_pred: np.ndarray) -> float:
    ...
    ⚠️ RECOMMENDED as PRIMARY metric for power price forecasting.
...
def print_metrics_report(
    ...
    if 'mase' in metrics and not np.isnan(metrics.get('mase', np.nan)):
        ...
        status = "✅ Better" if mase_val < 1 else "⚠️ Worse"
        print(f"MASE:                     {mase_val:>10.4f}  {status} than naive")

    if 'directional_accuracy' in metrics and not np.isnan(metrics.get('directional_accuracy', np.nan)):
