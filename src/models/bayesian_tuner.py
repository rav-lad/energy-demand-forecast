...
[OK] Adapts to regime changes (normal vs volatile markets)
[OK] More efficient: finds good params with fewer evaluations
[OK] Principled uncertainty quantification
[OK] Can run continuously in production
...
class AdaptiveWalkForwardTuner:
    ...
    def walk_forward_with_adaptive_tuning(
        ...
        first_pred_date = (
            first_test_datetime.normalize() + pd.Timedelta(days=1)
        ).date()
        ...
        while current_date <= df["datetime_hour"].max():
            ...
            day_mask = (df["datetime_hour"] >= current_date) & (
                df["datetime_hour"] < next_date
            )
            ...
            if len(test_df) != forecast_horizon_hours:
                if verbose and len(test_df) > 0:
                    logger.info(
                        f"Skipping {current_date.date()}: only {len(test_df)}/24 hours"
                    )
            ...
            daily_metrics_list.append(
                {"datetime": current_date, "mae": mae, "iteration": iteration}
            )
            ...
            if days_since_tuning >= self.tune_frequency_days:
                ...
                if verbose:
                    logger.info(
                        f"[OK] New model trained with updated hyperparameters on {len(train_df)} hours"
                    )
        ...
        from src.models.walk_forward_validator import calculate_price_forecast_metrics
...
if __name__ == "__main__":
    ...
    print("\n[OK] Bayesian optimization test completed!")
