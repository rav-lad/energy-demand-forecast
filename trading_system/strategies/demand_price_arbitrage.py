...
class DemandPriceArbitrageStrategy:
    ...
    def generate_signals(
        ...
        signals_df['demand_percentile'] = (
            predicted_demand.rank(pct=True)
        )
        ...
        signals_df['residual_load'] = (
            predicted_demand * (1 - signals_df['renewable_share'])
        )
        ...
        signals_df.loc[sell_mask, 'confidence'] = (
            ...
            (1 - self.renewable_high_threshold) * 0.5
        )
        ...
        if self.use_confidence_scaling:
            ...
            signals_df['position_size'] = (
                self.base_position_size * (0.5 + signals_df['confidence'])
            )
        else:
    ...
    def analyze_signal_quality(
        ...
        buy_accuracy = (buy_signals['forward_return'] > 0).sum() / len(buy_signals) if len(buy_signals) > 0 else 0
        ...
        sell_accuracy = (sell_signals['forward_return'] < 0).sum() / len(sell_signals) if len(sell_signals) > 0 else 0
        ...
        metrics = {
            ...
            'total_accuracy': (buy_accuracy * len(buy_signals) + sell_accuracy * len(sell_signals)) /
                             (len(buy_signals) + len(sell_signals)) * 100 if (len(buy_signals) + len(sell_signals)) > 0 else 0
        }
...
if __name__ == '__main__':
    ...
    renewable = 0.3 + 0.2 * np.sin(2 * np.pi * t / 365 + np.pi/2) + np.random.randn(len(dates)) * 0.05
    ...
    strategy.calibrate(
        demand_series[:calibration_end],
        renewable_series[:calibration_end]
    )
