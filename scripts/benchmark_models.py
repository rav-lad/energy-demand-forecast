...
class ModelBenchmark:
    ...
    def _load_model(self, model_name):
        ...
        if model_name == 'xgboost':
            ...
            for target in ['conso_elec_mw', 'conso_gaz_mw']:
                for q in [5, 50, 95]:
                    key = f"{target}_q{q}"
                    model_file = models_dir / "Quantile" / "lightgbm_quantile" / f"{target}_q{q}_{self.frequency}_withoutlags.pkl"
    ...
    def _prepare_features(self, model_name):
        ...
        if model_name in ['xgboost', 'ridge', 'lasso']:
            ...
            transformed_file = project_root / "data" / "transformed_data" / f"test_{self.frequency}_reglin_xgboost.csv"
            ...
            transformed_file = project_root / "data" / "transformed_data" / f"test_{self.frequency}_lightgbm_withoutlags.csv"
    ...
    def _print_summary(self):
        ...
        print(f"{'Model':<15} {'RMSE Elec':<12} {'RMSE Gaz':<12} {'R² Elec':<10} {'Inference (s)':<15}")
        print("-" * 70)
        ...
        for _, row in df.iterrows():
            ...
            print(f"{model:<15} {rmse_elec:<12.2f} {rmse_gaz:<12.2f} {r2_elec:<10.4f} {inference:<15.3f}")
