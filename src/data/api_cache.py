...
class ApiCache:
    ...
    def clear(self, older_than_days: Optional[int] = None):
        ...
        if older_than_days is None:
            ...
            logger.info(
                f"Cleared {cleared_count} old cache entries (>{older_than_days} days)"
            )
...
def main():
    ...
    parser.add_argument(
        ...
        nargs="?",
        const=-1,
        help="Clear cache (optionally older than N days)",
    )
    ...
    if args.clear is not None:
        if args.clear == -1:
            print("\n[CLEAR] Clearing all cache...")
            cache.clear()
        else:
            print(f"\n[CLEAR] Clearing cache older than {args.clear} days...")
