...
# Source: RTE Annual Electricity Report
...
def calculate_wind_power_from_speed(
    ...
    power_curve[mask_ramp] = (
        (wind_speed_ms[mask_ramp] - cut_in) / (rated - cut_in)
    ) ** 3
...
def calculate_solar_power_from_radiation(
    ...
    production_mw = production_fraction * capacity_mw * performance_ratio
...
def calculate_renewable_forecasts(
    ...
    df["wind_production_forecast_mw"] = calculate_wind_power_from_speed(
        wind_speed, capacity_mw=wind_capacity_mw
    )
    ...
    hour = (
        pd.to_datetime(df["datetime_hour"]).dt.hour.values
        if "datetime_hour" in df.columns
        else None
    )

    df["solar_production_forecast_mw"] = calculate_solar_power_from_radiation(
        solar_radiation, capacity_mw=solar_capacity_mw, hour=hour
    )
    ...
    logger.info(
        f"[OK] Calculated renewable production forecasts\n"
...
def calculate_net_load_forecast(
    ...
    df["renewable_curtailment_mw"] = np.maximum(df[renewable_col] - df[load_col], 0)

    # Renewable coverage (what % of load is covered by renewables)
    df["renewable_coverage_pct"] = np.minimum(
        df[renewable_col] / df[load_col] * 100, 100
    )

    logger.info(
        f"[OK] Calculated net load forecast\n"
...
def add_renewable_features(
    df: pd.DataFrame, include_net_load: bool = True
) -> pd.DataFrame:
    ...
    df["wind_capacity_factor"] = df["wind_production_forecast_mw"] / WIND_CAPACITY_MW

    # Solar capacity factor
    df["solar_capacity_factor"] = df["solar_production_forecast_mw"] / SOLAR_CAPACITY_MW

    # Residual load intensity (net load relative to typical)
    typical_load = 50000
    df["residual_load_intensity"] = (
        df.get("net_load_forecast_mw", df.get("load_forecast", typical_load))
        / typical_load
    )
    ...
    df = pd.DataFrame(
        {
            ...
            "solar_radiation_forecast": np.maximum(
                0,
                np.random.normal(300, 200, len(dates))
