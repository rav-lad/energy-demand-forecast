...
def rolling_forecast_lightgbm(freq="daily", lags="with"):
    ...
    for current_date in dates:
        ...
        df_trans = transform_lightgbm_quantile(
            df_window, frequency=freq, save=False, lags=(lags == "with")
        )
        ...
        for region in regions_today:
            ...
            X = row.drop(
                columns=["date", "insee_region", "conso_elec_mw", "conso_gaz_mw"],
                errors="ignore",
            )
            X = X.reindex(columns=expected_features, fill_value=0)

            out = {"date": current_date, "insee_region": region}
            ...
            for target in ["conso_elec_mw", "conso_gaz_mw"]:
                for q in [10, 50, 90]:
                    ...
                    if q == 50 and lags == "with":
                        idx = (df_full["date"] == current_date) & (
                            df_full["insee_region"] == region
                        )
