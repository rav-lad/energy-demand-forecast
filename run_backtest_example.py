...
def run_backtest(data, config):
    ...
    logger.info(f"\nData split:")
    logger.info(f"  Calibration: {calibration_data.index[0].date()} to {calibration_data.index[-1].date()}")
    ...
    strategy = DemandPriceArbitrageStrategy(
        ...
        renewable_high_threshold=strategy_config.get('signals', {}).get('renewable_threshold_high', 0.7),
        renewable_low_threshold=strategy_config.get('signals', {}).get('renewable_threshold_low', 0.3),
        base_position_size=strategy_config.get('position_sizing', {}).get('base_size', 1000),
        use_confidence_scaling=strategy_config.get('position_sizing', {}).get('scale_with_confidence', True)
    )
    ...
    strategy.calibrate(
        calibration_data['predicted_demand'],
        calibration_data['renewable_share']
    )
    ...
    signals_df = strategy.generate_signals(
        test_data['predicted_demand'],
        test_data['renewable_share']
    )
    ...
    signal_quality = strategy.analyze_signal_quality(
        signals_df,
        test_data['price_FR']
    )
...
def main():
    ...
    try:
        # Load configuration
        config = get_config()
        logger.info(f"Configuration loaded: {config.get('project.name')} v{config.get('project.version')}")
