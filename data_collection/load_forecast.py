...
def generate_load_forecast_simple(
    ...
    hour_factors = {
        0: 0.75, 1: 0.70, 2: 0.68, 3: 0.67, 4: 0.68, 5: 0.72,
        6: 0.80, 7: 0.90, 8: 0.95, 9: 1.00, 10: 1.02, 11: 1.03,
        12: 1.00, 13: 0.98, 14: 0.97, 15: 0.96, 16: 0.98, 17: 1.05,
        18: 1.10, 19: 1.12, 20: 1.10, 21: 1.05, 22: 0.95, 23: 0.85
    }
    ...
    df["load_forecast"] = (
        ...
        * df["hour_factor"]
        * df["dow_factor"]
        + temp_sensitivity * (df["hdd"] * 1.2 + df["cdd"] * 0.8)  # Heating stronger than cooling
    )
    ...
    logger.info(
        f"✅ Generated simple load forecast\n"
...
def generate_load_forecast_ml(
    ...
    logger.info(f"✅ Trained ML load forecast model (R²: {model.score(X_train_scaled, y_train):.3f})")
...
def simulate_load_forecast_from_actual(
    ...
    logger.info(
        f"✅ Simulated load forecast from actual values\n"
...
def get_load_forecast_for_price_forecasting(
    ...
    if mode == "simple":
        ...
        if df_historical_load is None:
            logger.warning("No historical load data provided, falling back to simple model")
            return get_load_forecast_for_price_forecasting(
                df_weather, None, mode="simple", forecast_horizon_hours=forecast_horizon_hours
            )
        ...
        return simulate_load_forecast_from_actual(
            df_merged,
            forecast_horizon_hours
        )
...
if __name__ == "__main__":
    ...
    df_historical = pd.DataFrame({
        ...
        "load_mw": 50000 + np.random.normal(0, 5000, len(pd.date_range("2023-01-01", "2023-12-31", freq="H"))),
        "temp_actual": np.random.normal(12, 8, len(pd.date_range("2023-01-01", "2023-12-31", freq="H")))
    })
