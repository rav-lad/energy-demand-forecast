...
def plot_predictions(
    ...
    axes[0].plot(dates.iloc[idx], y_pred[idx], label="Predicted", alpha=0.7, linewidth=1.5)
...
def plot_quantile_predictions(
    ...
    ax.plot(dates.iloc[idx], y_true[idx], label="Actual", linewidth=2, color="black", alpha=0.7)
    ax.plot(
        dates.iloc[idx], pred_median[idx], label="Median Forecast", linewidth=2, color="blue"
    )
    ax.fill_between(
        dates.iloc[idx],
        pred_lower[idx],
...
def train_ensemble_model(
    ...
    if track_mlflow:
        ...
        # Log model
        tracker.log_model(model, "model", registered_model_name="price_forecast_ensemble")
...
def walk_forward_validation(
    df: pd.DataFrame, feature_cols: list, n_splits: int = 5
):
    ...
    print("=" * 70)
    print(f"Average Test RMSE: {results_df['rmse'].mean():.2f} ± {results_df['rmse'].std():.2f}")
    print(f"Average Test R²:   {results_df['r2'].mean():.4f} ± {results_df['r2'].std():.4f}")
    print(f"Average Test MAPE: {results_df['mape'].mean():.2f}% ± {results_df['mape'].std():.2f}%")
