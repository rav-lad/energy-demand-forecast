...
page = st.sidebar.radio(
    "Navigation",
    ["📊 Overview", "🎯 Predictions", "💹 Trading Signals", "📈 Backtests", "🔬 Model Comparison"]
)
...
# =============================================================================
    ...
    if 'actual_elec' in predictions_df.columns and 'pred_elec' in predictions_df.columns:
        metrics_elec = calculate_metrics(
            predictions_df['actual_elec'].values,
            predictions_df['pred_elec'].values
        )

        metrics_gaz = calculate_metrics(
            predictions_df['actual_gaz'].values,
            predictions_df['pred_gaz'].values
        )
...
# =============================================================================
    ...
    metrics_normalized['rmse_norm'] = 1 - (metrics_normalized['rmse_elec'] - metrics_normalized['rmse_elec'].min()) / (metrics_normalized['rmse_elec'].max() - metrics_normalized['rmse_elec'].min())
    metrics_normalized['r2_norm'] = metrics_normalized['r2_elec']
    metrics_normalized['speed_norm'] = 1 - (metrics_normalized['training_time_sec'] - metrics_normalized['training_time_sec'].min()) / (metrics_normalized['training_time_sec'].max() - metrics_normalized['training_time_sec'].min())
