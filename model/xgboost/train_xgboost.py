...
def train_xgboost(frequency: str):
    ...
    # 2) Transform using pre-fitted scaler (from gridsearch)
    ...
    mse_elec = mean_squared_error(y.iloc[:, 0], y_pred[:, 0])
    mse_gaz = mean_squared_error(y.iloc[:, 1], y_pred[:, 1])
    print(f"Electricity MSE: {mse_elec:.4f} | RMSE: {np.sqrt(mse_elec):.4f}")
    print(f"Gas MSE: {mse_gaz:.4f} | RMSE: {np.sqrt(mse_gaz):.4f}")

    # 5) Save model
    joblib.dump(model, MODELS_DIR / f"xgb_{frequency}.pkl")
    print(f"XGBoost model saved ({frequency})")
