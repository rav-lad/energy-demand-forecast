...
def transform_regression_and_xgb(
    ...
    df["radiation_et0_ratio"] = df["shortwave_radiation_sum"] / (
        df["et0_fao_evapotranspiration"] + 1e-5
    )
    df["sunshine_ratio"] = df["sunshine_duration"] / (df["daylight_duration"] + 1e-5)

    # One-hot encoding
    df = pd.get_dummies(
        df, columns=["weather_code", "wind_sector", "insee_region"], drop_first=True
    )
...
def transform_lightgbm_quantile(
    ...
    df[["weather_code", "wind_sector", "insee_region"]] = df[
        ["weather_code", "wind_sector", "insee_region"]
    ].astype(str)
    df = pd.get_dummies(
        df, columns=["weather_code", "wind_sector", "insee_region"], drop_first=True
    )
    ...
    if save:
        ...
        df.to_csv(
            TRANSFORMED_DIR / f"train_{frequency}_lightgbm_quantile_{suffix}.csv",
            index=False,
        )
