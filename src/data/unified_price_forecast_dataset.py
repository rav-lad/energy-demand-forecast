...
from src.data.weather_forecast import get_weather_for_price_forecasting
from src.data.load_forecast import get_load_forecast_for_price_forecasting
from src.data.renewable_forecast import add_renewable_features
from src.models.data_loader import add_calendar_features
...
def load_historical_prices(
    ...
    # Filter date range
    df = df[(df["datetime_hour"] >= start_date) & (df["datetime_hour"] <= end_date)]

    df = df[["datetime_hour", "price", "load_actual"]].copy()

    logger.info(
        f"[OK] Loaded historical prices\n"
        f"   Records: {len(df):,}\n"
...
def create_lag_features(
    ...
    IMPORTANT: lags are computed by TIME (merge on datetime - lag_hours), NOT by
    positional shift(). Using shift() on a dataset with missing hours produces
    incorrect lags (e.g. shift(24) may point to t-22h instead of t-24h when
    hours are missing). The merge approach guarantees correct temporal alignment.

    Args:
        df: DataFrame sorted by datetime, with 'datetime_hour' column
    ...
    if invalid_lags:
        logger.warning(
            f"Found lags < forecast horizon ({forecast_horizon}h): {invalid_lags} — removing."
        )
    ...
    for col in target_cols:
        ...
        for lag in lags:
            # Merge on real timestamps — correct even when hours are missing
            ref = df[["datetime_hour", col]].copy()
            ref["datetime_hour"] = ref["datetime_hour"] + pd.Timedelta(hours=lag)
            ref = ref.rename(columns={col: f"{col}_lag_{lag}h"})
            df = df.merge(ref, on="datetime_hour", how="left")

    # Rolling statistics over a time-based window, shifted by forecast_horizon
    # Uses only data available at prediction time (t - forecast_horizon and earlier)
    df = df.set_index("datetime_hour").sort_index()
    for col in target_cols:
        if col not in df.columns:
            continue
        # Shift by forecast_horizon hours before computing rolling stats
        shifted = df[col].shift(freq=pd.Timedelta(hours=forecast_horizon))
        for window in [24, 168]:
            df[f"{col}_roll_mean_{window}h"] = shifted.rolling(
                f"{window}h", min_periods=window // 2
            ).mean()
            df[f"{col}_roll_std_{window}h"] = shifted.rolling(
                f"{window}h", min_periods=window // 2
            ).std()
    df = df.reset_index()

    logger.info(
        f"Created lag features (time-based merge, no positional shift)\n"
        f"   Columns: {target_cols}, Lags: {lags}h, Horizon: {forecast_horizon}h"
    )
...
def align_forecast_features(
    ...
    df_hist["datetime_hour"] = pd.to_datetime(df_hist["datetime_hour"]).dt.tz_localize(
        None
    )
    df_fcst["datetime_hour"] = pd.to_datetime(df_fcst["datetime_hour"]).dt.tz_localize(
        None
    )
    ...
    df_fcst["datetime_hour"] = df_fcst["datetime_hour"] - timedelta(
    ...
    logger.info(
        f"[OK] Aligned forecast features with {forecast_horizon_hours}h horizon\n"
...
def build_unified_price_forecasting_dataset(
    ...
    logger.info(
        f"\n" + "=" * 80 + "\n"
        f"[START] BUILDING UNIFIED PRICE FORECASTING DATASET\n"
        f"=" * 80 + "\n"
    ...
    # =========================================================================
    logger.info("\n[STATS] STEP 1/6: Loading historical prices and load...")
    df = load_historical_prices(start_date, end_date)

    # =========================================================================
    # STEP 2: Get weather forecasts
    # =========================================================================
    logger.info("\n[WEATHER]  STEP 2/6: Getting weather forecasts...")
    df_weather = get_weather_for_price_forecasting(
        start_date, end_date, mode=mode, forecast_horizon_hours=forecast_horizon_hours
    )

    # =========================================================================
    # STEP 3: Generate load forecasts
    # =========================================================================
    logger.info("\n[POWER] STEP 3/6: Generating load forecasts...")
    ...
    logger.info(
        f"   Weather forecasts: {len([c for c in forecast_features if 'temp' in c or 'wind' in c or 'solar' in c])}"
    )
    logger.info(
        f"   Load forecast: {len([c for c in forecast_features if 'load' in c])}"
    )
    ...
    print(
        df[
            [
                ...
                "wind_production_forecast_mw",
                "solar_production_forecast_mw",
            ]
        ].head(10)
    )

    print("\n" + "=" * 80)
    print("[CHART] FEATURE SUMMARY")
    print("=" * 80)
