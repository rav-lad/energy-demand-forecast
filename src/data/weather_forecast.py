...
def get_weather_forecast_realtime(
    lat: float = 48.8566, lon: float = 2.3522, forecast_days: int = 2
) -> pd.DataFrame:
    ...
    try:
        ...
        logger.info(f"[OK] Retrieved {len(df)} hours of weather forecast")
        return df
...
def get_historical_weather_with_simulated_forecast(
    ...
    try:
        ...
        logger.info(
            f"[OK] Retrieved {len(df)} hours of historical weather with simulated forecasts\n"
...
def add_realistic_forecast_errors(
    df: pd.DataFrame, horizon_hours: int = 24
) -> pd.DataFrame:
    ...
    wind10m_abs_error = df["wind10m_actual"] * wind10m_rel_error + np.random.normal(
        0, 1.5, len(df)
    )
    ...
    wind100m_abs_error = df["wind100m_actual"] * wind100m_rel_error + np.random.normal(
        0, 2.0, len(df)
    )
    ...
    df["solar_radiation_forecast"] = np.maximum(
        0, df["solar_radiation_actual"] + solar_error
    )
    ...
    df["precipitation_forecast"] = np.maximum(
        0, df["precipitation_actual"] + precip_error
    )
    ...
    logger.info(
        f"[OK] Added realistic forecast errors for {horizon_hours}h horizon\n"
...
def aggregate_regional_weather(
    df_weather: pd.DataFrame, regions: list = None
) -> pd.DataFrame:
    ...
    logger.info(
        f"[OK] Aggregated weather data across {df_weather['insee_region'].nunique()} regions"
    )
...
def get_weather_for_price_forecasting(
    ...
    if mode == "backtest":
        ...
        df = get_historical_weather_with_simulated_forecast(
            start_date, end_date, forecast_horizon_hours=forecast_horizon_hours
        )
    elif mode == "realtime":
...
if __name__ == "__main__":
    ...
    print(
        f"Temperature MAE: {np.abs(df['temp_forecast'] - df['temp_actual']).mean():.2f}°C"
    )
    print(
        f"Wind MAE: {np.abs(df['wind100m_forecast'] - df['wind100m_actual']).mean():.2f} m/s"
    )
