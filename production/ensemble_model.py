...
from production.config import MODEL_PARAMS, ENSEMBLE_WEIGHTS


class EnsembleForecaster:
    """Ensemble forecaster combining multiple models."""

    def __init__(self, models: List[str] = ["ridge", "xgboost"], weights: Dict[str, float] = None):
        """
        ...
        if set(models) != set(self.weights.keys()):
            raise ValueError(f"Weights keys {self.weights.keys()} don't match models {models}")
...
def train_ensemble_walk_forward(
    ...
    """
    from model.price_forecasting.walk_forward_validator import walk_forward_validate
