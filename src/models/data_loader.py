...
def validate_real_price_data(df: pd.DataFrame, data_source: str = "unknown") -> None:
    ...
    if len(prices) < 1000:
        logger.warning(
            f"[WARNING]  Only {len(prices)} price observations - may be insufficient for production\n"
    ...
    if extreme_negative > len(prices) * 0.01:  # More than 1%
        logger.warning(
            f"[WARNING]  Many extreme negative prices detected: {extreme_negative} observations < -100 EUR/MWh\n"
    ...
    if cv < 0.1:
        logger.warning(
            f"[WARNING]  Low price volatility detected (CV={cv:.3f})\n"
    ...
    if max_repeats > 10:
        ...
        logger.warning(
            f"[WARNING]  Repeated price values detected\n"
    ...
    if "datetime_hour" in df.columns:
        ...
        if coverage < 0.90:
            logger.warning(
                f"[WARNING]  Low timestamp coverage: {coverage:.1%}\n"
    ...
    logger.info(
        f"[OK] Price data validation PASSED for '{data_source}'\n"
...
def simulate_realistic_prices(
    ...
    if "insee_region" in df.columns:
        df = df.groupby("datetime_hour").agg({"conso_elec_mw": "sum"}).reset_index()
    ...
    for t in range(1, n):
        ...
        vol[t] = np.sqrt(
            0.05 * volatility**2 + 0.1 * noise[t - 1] ** 2 + 0.85 * vol[t - 1] ** 2
        )
...
def load_price_and_load_data(
    ...
    simulate_prices: bool = False,  # [OK] CHANGED: DEFAULTS TO REAL DATA
    random_seed: int = 42,
    price_file: str = "data/raw_data/market_prices/day_ahead_prices_FR.csv",
    strict_validation: bool = True,  # [OK] NEW: Crash on simulated data in production
) -> pd.DataFrame:
    """Load energy load data and corresponding prices.

    [WARNING] CRITICAL: This function MUST use REAL prices from ENTSO-E by default.
    ...
    """
    # [OK] CRITICAL SAFETY CHECK - PREVENT SIMULATED PRICES IN PRODUCTION
    if simulate_prices and strict_validation:
        raise RuntimeError(
            "\n" + "=" * 80 + "\n"
            "[ERROR] CRITICAL ERROR: simulate_prices=True is NOT allowed in production!\n"
            "\n"
            "Simulated prices are only for unit testing.\n"
            "They lack:\n"
    ...
    # [OK] EXPLICIT WARNING IF USING SIMULATED PRICES
    if simulate_prices:
        logger.warning(
            "\n" + "=" * 80 + "\n"
            "[WARNING]  WARNING: Using SIMULATED prices!\n"
            "\n"
    ...
    if simulate_prices:
        ...
        # [OK] VALIDATE REAL PRICE DATA (CRITICAL SAFETY CHECK)
        if strict_validation:
            validate_real_price_data(merged_df, data_source=f"ENTSO-E ({price_file})")
...
def add_lag_features(
    ...
    """
    # For 24h-ahead forecasting only lags >= 24h are valid.
    # Lags 1h/2h/3h would be data leakage (price at t-1 is not available
    # when predicting t+24). They are removed here.
    if lags is None:
        lags = [24, 48, 72, 168]  # minimum lag = forecast horizon (24h)

    df = df.copy()

    # Use time-based merge (not positional shift) to handle missing hours correctly.
    # shift(n) on a dataset with gaps points to the wrong timestamp.
    if "datetime_hour" in df.columns:
        for lag in lags:
            ref = df[["datetime_hour", target_col]].copy()
            ref["datetime_hour"] = ref["datetime_hour"] + pd.Timedelta(hours=lag)
            ref = ref.rename(columns={target_col: f"{target_col}_lag_{lag}h"})
            df = df.merge(ref, on="datetime_hour", how="left")
    else:
        # Fallback: positional shift (only safe on gapless hourly data)
        for lag in lags:
            df[f"{target_col}_lag_{lag}h"] = df[target_col].shift(lag)

    # Rolling statistics shifted by 24h (forecast horizon) — no leakage.
    # shift(1) would include prices from t-1 which are unknown at t when predicting t+24.
    for window in [24, 168]:  # 1 day, 1 week
        df[f"{target_col}_roll_mean_{window}h"] = (
            df[target_col].shift(24).rolling(window=window, min_periods=1).mean()
        )
        df[f"{target_col}_roll_std_{window}h"] = (
            df[target_col].shift(24).rolling(window=window, min_periods=1).std()
        )
...
def prepare_price_forecasting_dataset(
    ...
    simulate_prices: bool = False,  # [OK] NEW: Explicit control (defaults to REAL data)
    strict_validation: bool = True,  # [OK] NEW: Safety check for production
    random_seed: int = 42,
) -> Tuple[pd.DataFrame, list]:
    """Prepare complete dataset for price forecasting.

    [WARNING] CRITICAL: Uses REAL ENTSO-E prices by default.
    ...
    feature_cols = [
        col for col in df.columns if col not in ["datetime_hour", target_col, "load_mw"]
    ]

    logger.info(f"Total features: {len(feature_cols)}")
    logger.info(
        f"Features include load lags but NOT contemporaneous load (avoiding leakage)"
    )
...
def prepare_price_forecasting_with_fuel_prices(
    data_path: str = "data/modified_data/energy_hourly_regional.csv",
    target_col: str = "price",
    include_fuel_prices: bool = False,  # TTF/EUA/coal require paid data feeds (ICE, Bloomberg)
    random_seed: int = 42,
) -> Tuple[pd.DataFrame, list]:
    ...
    if include_fuel_prices:
        from src.data.fuel_prices import generate_fuel_price_features
        ...
        if duplicate_cols:
            logger.warning(
                f"Duplicate columns found: {duplicate_cols}. Dropping from fuel_df."
            )
