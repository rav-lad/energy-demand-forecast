...
def calculate_price_forecast_metrics(y_true: np.ndarray, y_pred: np.ndarray) -> Dict[str, float]:
...
def walk_forward_validate(
    ...
    if initial_train_end_idx >= len(df):
        raise ValueError(f"Not enough data: need {initial_train_days} days for initial training")
    ...
    if verbose:
        logger.info(
            ...
            f"Date range: {train_df['datetime_hour'].min()} to {train_df['datetime_hour'].max()}\n"
    ...
    while current_date <= df['datetime_hour'].max():
        ...
        if len(test_df) != forecast_horizon_hours:
            if verbose and len(test_df) > 0:
                logger.info(f"Skipping {current_date.date()}: only {len(test_df)}/24 hours available")
        ...
        if days_since_retrain >= retrain_frequency_days:
            ...
            if verbose and iteration % 10 == 0:
                logger.info(
                    ...
                    f"({train_df['datetime_hour'].min()} to {train_df['datetime_hour'].max()})"
                )
    ...
    overall_metrics['coverage_hours'] = (
        predictions_df['datetime_hour'].max() - predictions_df['datetime_hour'].min()
    ).total_seconds() / 3600
    ...
    if verbose:
        logger.info(
            ...
            f"Date range: {predictions_df['datetime_hour'].min()} to {predictions_df['datetime_hour'].max()}\n"
            f"\n"
            f"Overall Metrics:\n"
...
def plot_walk_forward_results(
    ...
    fig.suptitle(f'{model_name} - Walk-Forward Validation Results', fontsize=16, fontweight='bold')

    # 1. Actual vs Predicted (time series)
    ax = axes[0, 0]
    sample_size = min(7*24, len(predictions_df))  # Show first week
    sample_df = predictions_df.head(sample_size)
    ax.plot(sample_df['datetime_hour'], sample_df['actual'], label='Actual', alpha=0.7, linewidth=1.5)
    ax.plot(sample_df['datetime_hour'], sample_df['predicted'], label='Predicted', alpha=0.7, linewidth=1.5)
    ...
    if daily_metrics:
        ...
        ax.plot(daily_df['datetime'], daily_df['mae'], linewidth=1.5, marker='o', markersize=3)
        ax.axhline(daily_df['mae'].mean(), color='red', linestyle='--', linewidth=2,
    ...
    table = ax.table(cellText=metrics_summary, loc='center', cellLoc='left',
                     colWidths=[0.5, 0.5])
